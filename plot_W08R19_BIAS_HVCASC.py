...
def parse_args():
    ...
    for i in range(994, 1009):
        if i<=999:
            add_arg(f'-file{i}', default=f'/home/bgnet/tbsw_workspace_july_2023/Plotter_cal/Plotter-run000{i}-roi-450-470-2-510.root', type=str,
                help=f'data or simulation file {i} with path')
        else:
            add_arg(f'-file{i}', default=f'/home/bgnet/tbsw_workspace_july_2023/Plotter_cal/Plotter-run00{i}-roi-450-470-2-510.root', type=str,
                help=f'data or simulation file {i} with path')
...
pdf_file = ROOT.TPDF(os.path.join(output_folder, f"W08_R19_BIAS_HVCASC_994-1008.pdf"))
...
for log_scale in [False, True]:
    ...
    hCCharges[0].SetTitle(f"BIAS_HVCASC 994-1008")
    ...
    legend.AddEntry(hCCharges[0], f"BIAS_HVCASC 2", "l")
    for i in range(1, 15):
        legend.AddEntry(hCCharges[i], f"BIAS_HVCASC {i*2+2}", "l")
    legend.Draw()
...
for i, h in enumerate(hCCharges):
    ...
    h.SetStats(0)
    h.Draw("HIST")
    h.SetTitle(f"BIAS_HVCASC {i*2+2}")
...
hCCharges[0].Draw("HIST")
hCCharges[0].SetTitle(f"BIAS_HVCASC 994")
hCCharges[0].GetXaxis().SetTitle("Cluster Charge [electrons]")
hCCharges[0].GetYaxis().SetTitle("Number of Hits")
hCCharges[0].GetYaxis().SetTitleOffset(1.2)
...
hCCharges[1].SetTitle(f"BIAS_HVCASC 996")
...
legend = ROOT.TLegend(0.7, 0.7, 0.9, 0.9)
legend.AddEntry(hCCharges[1], f"BIAS_HVCASC 4", "l")
for i in range(3, 14, 2):
    legend.AddEntry(hCCharges[i], f"BIAS_HVCASC {i*2+2}", "l")
legend.Draw()
